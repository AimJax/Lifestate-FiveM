...
let moduleMap = {
  "./config": () => {
    ...
    return __federation_import("./__federation_expose_Config-BO0so3cw.js").then((module) => Object.keys(module).every((item) => exportSet.has(item)) ? () => module.default : () => module);
  }
};
const seen = {};
const dynamicLoadingCss = (cssFilePaths, dontAppendStylesToHead, exposeItemName) => {
  ...
  cssFilePaths.forEach((cssPath) => {
    ...
    if (dontAppendStylesToHead) {
      const key = "css__npwd_lifestate_ojol_customer__" + exposeItemName;
